...
{CONTENT_RSS}
	...
	function __construct() // PHP 8 : un constructeur ne peut plus porter le nom de la classe
	{
	...
	function template()
	{
		...
		if($action == "" && !$preview && ((template_match("plugin:COMMENTS", $html, $null) && strpos($CON, "{NO_COMMENTS}") === false)
			...
			$tmpl = strtr($tmpl, array(
				...
				"{FORM_NAME_VALUE}" => !empty($comment_captcha_failed) ? h($_POST["name"] ?? "") : "",
				"{FORM_EMAIL_VALUE}" => !empty($comment_captcha_failed) ? h($_POST["email"] ?? "") : "",
				"{FORM_CONTENT_VALUE}" => !empty($comment_captcha_failed) ? h($_POST["content"] ?? "") : "",
			...
			$filenames = array(); // sinon count($filenames) plus bas = TypeError en PHP 8 si le dossier n'existe pas
			if($dir = @opendir($this->comments_dir . $page)) {
				...
				foreach($filenames as $filename) {
					...
					list($ip, $name, $email) = array_pad(explode("\t", $meta), 3, ""); // evite Undefined offset
	...
	function writeRSS($page, $id, $content)
	{
		...
		$pagelink = (!empty($_SERVER["HTTPS"]) ? "https://" : "http://") . ($_SERVER["SERVER_NAME"] ?? "") . ":" . ($_SERVER["SERVER_PORT"] ?? "") . ($_SERVER["SCRIPT_NAME"] ?? "");

		preg_match("/<\/language>(.*)<\/channel>/s", @file_get_contents($this->rss_file), $matches);

		$items = $matches[1] ?? ""; // pas de match si le fichier RSS est vide
