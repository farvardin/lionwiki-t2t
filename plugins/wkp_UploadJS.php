			...
			if($opening_dir = @opendir($abs_dir)) {
				...
				foreach($files as $file) {
					...
					if((authentified()) && ($file[0] != '..'))
						$this->uploadjs .= '<td><span style="cursor: pointer" onclick="parent.document.getElementById(\'nameImg\').innerHTML = \''. $file[0] .'\';">'. $this->TP_FILE_CHOOSE .'</span></td><td><a title="delete" href="'.$self.$templateUpload.'&del=' . u($path) . "&curdir=" . u($rel_dir) . '">&times;</a></td>';
					else
