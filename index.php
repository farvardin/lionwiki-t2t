...
$SYNTAX_PAGE = '?page=help_syntax.en';
...
//if(get_magic_quotes_gpc()) // magic_quotes_gpc has been removed as of PHP 5.4.0
...
// remove underscore in title display:
$TITLE = str_replace('_', ' ', $TITLE);
// remove color information in title display
$TITLE = preg_replace('/#C(\w+)#/i', ' ', $TITLE);
$CON = $content;
$error = h($error);
...
if(!$action || $preview) { // page parsing
    ...
    $CON = preg_replace("/===(.*)===/m", "!!$1", $CON);
    $CON = preg_replace("/==(.*)==/m", "!$1", $CON);
    // TODO: remove this? we don't need to alter H1 => h2
    ...
    // images
    preg_match_all("#\[((https?://|\./)[^|\]\"]+\.(jpeg|jpg|gif|png|svg))(\|[^\]]+)?\]#", $CON, $imgs, PREG_SET_ORDER);
...
//new:
 $html = file_exists($TEMPLATE) ? file_get_contents($TEMPLATE) : fallback_template();
...
$tpl_subs = array(
    ...
    'EDIT#' => !$action ? ("<a href=\"$self?page=".u($page)."&amp;action=edit".(is_writable("$PG_DIR$page.txt") ? "\">#</a>" : "&amp;showsource=1\">$T_SHOW_SOURCE</a>")) : "",
...
function diff($f1, $f2)
{
    // executes either builtin simple diff or complex diff plugin, if present ...
...
progress {  color: #586e75;  background-color: #eee;  border-radius: 0px; border: solid 1px;  width: 10em;  height: 1em;}
progress::-moz-progress-bar {  background: #586e75;}
progress::-webkit-progress-value {  background: #586e75;}
...
<body>
<table border="0" width="100%" cellpadding="4" class="main">
<tr>
	<td colspan="2">{HOME} {RECENT_CHANGES}</td>
	<td style="text-align:right">{EDIT} {SYNTAX} {HISTORY}</td>
</tr>
<tr><th colspan="3" class="main"><hr/><h1 id="page-title">{PAGE_TITLE} {<span class="pageVersionsList">( plugin:VERSIONS_LIST )</span>}</h1></th></tr>
<tr>
...
<tr><td colspan="3"  class="main><hr/></td></tr>
<tr>
	<td class="main><hr/><div>{SEARCH_FORM}{SEARCH_INPUT}{SEARCH_SUBMIT}{/SEARCH_FORM}</div><hr/><br/>{LAST_CHANGED_TEXT}: {LAST_CHANGED} {COOKIE}<br/>Powered by <a href="https://lionwiki-t2t.sourceforge.io/">LionWiki-t2t</a>. </br>{EDIT} {SYNTAX} {HISTORY}</td>
	<td class="main"></td>
